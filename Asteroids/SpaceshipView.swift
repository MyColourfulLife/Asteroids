...
class SpaceshipView: UIView {
    ...
    var shieldLevel:Double = 100 {
        didSet{
            shieldLevel = min(max(shieldLevel,0),100)
            sheildLevelChanged()
        }
    }
    ...
    func shieldBoundary(in view:UIView)->UIBezierPath {
        return getSheildPath(in:view)
    }
    ...
    private var shieldLinewidth:CGFloat = 1.0 {
        didSet{
            setNeedsDisplay()
        }
    }
    ...
    private func sheildLevelChanged() {
        if !exploading {
    ...
    private func updateImageViewFrame() {
        if !exploading && imageView.transform == CGAffineTransform.identity {
